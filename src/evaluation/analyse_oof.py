...
from src.config import OUTPUT_MODEL, SEED
...
def load_fold_predictions(
    ...
    # --------------------------------------------------------------
    # Existing CV files may contain only fusion probabilities.
    # If so, try to merge separately generated branch predictions.
    # --------------------------------------------------------------
...
def validate_oof(
    ...
    if not leaking_patients.empty:
        raise ValueError(
            "Some patients occur in several "
            "outer evaluation folds:\n"
            f"{leaking_patients.head()}"
        )
...
def calculate_fold_differences(
    metrics_df,
):
...
def main():
    ...
    # --------------------------------------------------------------
    # Fold metrics for all four models
    # --------------------------------------------------------------
    ...
    print(
        "\nRunning patient-cluster "
        f"bootstrap ({N_BOOTSTRAP} replicates)..."
    )
    ...
    bootstrap_df.to_csv(
        RESULTS_DIR
        / "patient_cluster_bootstrap.csv",
        index=False,
    )

    bootstrap_summary.to_csv(
        RESULTS_DIR
        / "bootstrap_95ci_summary.csv",
        index=False,
    )
