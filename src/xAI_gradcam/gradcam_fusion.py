...
# ------------------------------------------------------------------
# Préparation de la paire local/global
# ------------------------------------------------------------------
...
local_df = pd.read_csv(
    ...
    / (
        ...
        f"{param_inputs['zoom']['height']}x"
        f"{param_inputs['zoom']['width']}.csv"
    )
)
...
global_df = pd.read_csv(
    ...
    / (
        ...
        f"{param_inputs['full']['height']}x"
        f"{param_inputs['full']['width']}.csv"
    )
)
...
df = pd.merge(
    local_df,
    global_df[["lesion_key", "global_path"]],
    on="lesion_key",
    how="inner",
    validate="one_to_one",
)
...
gmic = tf.keras.models.load_model(
...
print("Entrées GMIC :", gmic.input_shape)
print("Sortie GMIC :", gmic.output_shape)
...
def make_gradcam_heatmap(
    ...
    local_extractor = model.get_layer(
        "local_feature_extractor"
    )

    global_extractor = model.get_layer(
    ...
    if branch == "local":
        conv_layer = local_extractor.get_layer(
            last_conv_layer_name
        )
        ...
        conv_outputs, local_features = cam_extractor(
            local_input,
            training=False,
        )

        global_features = global_extractor(
            global_input,
            training=False,
        )

    elif branch == "global":
        conv_layer = global_extractor.get_layer(
            last_conv_layer_name
        )
        ...
        conv_outputs, global_features = cam_extractor(
            global_input,
            training=False,
        )

        local_features = local_extractor(
            local_input,
            training=False,
        )
        ...
        raise ValueError(
            "branch doit valoir 'local' ou 'global'."
        )

    fused_features = model.get_layer(
        "feature_fusion"
    )([
        global_features,
        local_features,
    ...
    with tf.GradientTape() as tape:
        conv_outputs_value, predictions_value = grad_model(
            [local_array, global_array],
            training=False,
        )
    ...
    if gradients is None:
        raise RuntimeError(
            f"Impossible de calculer les gradients pour la branche {branch}."
        )
...
save_gradcam_overlay(
    image_batch=local_array,
    heatmap=local_heatmap,
    output_path="gradcam_local_overlay.png",
    title=f"Local Grad-CAM | {common_title}",
)

save_gradcam_overlay(
    image_batch=global_array,
    heatmap=global_heatmap,
    output_path="gradcam_global_overlay.png",
    title=f"Global Grad-CAM | {common_title}",
)