...
def build_global_model(
    ...
    x = tf.keras.layers.Flatten(name="resnet_global_flatten")(x)
    ...
    x = tf.keras.layers.BatchNormalization(name="mammography_adapter_batch_norm")(x)

    x = tf.keras.layers.ReLU(name="mammography_adapter_relu")(x)
